            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <form method="post" action="{{route('group.edit')}}">
                    ...
                    <div class="form-group">
                        <input class="form-control" name="name" type="text" placeholder="Name" value="{{old('name', $group->name)}}">
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control select2" style="width: 100%;" name="leader">
                            ...
                            @foreach($students as $student)
                                <option value="{{$student->id}}" @if($group->leader==$student->id) {{ 'selected' }} @endif>{{$student->name}}</option>
                            @endforeach
